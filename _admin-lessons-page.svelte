<script lang="ts">
  ...
  const filteredLessons = $derived(data.lessons.filter((lesson) => {
...
<AdminHeader title="Core lessons" description="Observed backend lesson progress enriched with source-controlled public catalog metadata where available.">{#snippet actions()}<a class="admin-button--secondary" href="/core" target="_blank" rel="noreferrer">Preview Core</a>{/snippet}</AdminHeader>
<AdminSectionCard eyebrow="Observed content" title="Lesson inventory" description="Admin API v1 reports lesson identifiers observed in progress records; title and preview metadata come from the existing frontend catalog.">
  {#if data.apiError}<AdminEmptyState state="error" title="Lesson data could not be loaded" detail={data.apiError} />
  {:else if data.dataSource === 'not_available'}<AdminEmptyState title="Lesson data is not available" detail="The backend reported data_source: not_available. No fallback lesson inventory is shown." />
  {:else}
