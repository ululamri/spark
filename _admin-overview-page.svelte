<script lang="ts">
  ...
  function statusTone(status: string) {
    return status === 'available' || status === 'connected' ? 'success' as const : status === 'unavailable' ? 'danger' as const : 'neutral' as const;
  }
</script>
...
{:else}
  <div class="admin-stat-grid">{#each metrics as metric}<AdminStatCard {...metric} />{/each}</div>
  ...
  <div class="admin-card-grid">
    <AdminSectionCard eyebrow="Runtime" title="System health" description="Safe status fields returned by the protected Admin API.">
      <div class="admin-status-card"><div class="admin-status-card__top"><strong>Spark API</strong><AdminStatusBadge label={data.overview.system_health.service} tone={statusTone(data.overview.system_health.service)} /></div><p>Database: {data.overview.system_health.database}.</p></div>
    </AdminSectionCard>
    <AdminSectionCard eyebrow="Starknet" title="Network status" description="No private RPC values are returned to the browser.">
      <div class="admin-status-card"><div class="admin-status-card__top"><strong>Read-only integration</strong><AdminStatusBadge label={data.overview.starknet_status.status} tone={statusTone(data.overview.starknet_status.status)} /></div><p>{data.overview.starknet_status.configured_networks.length ? data.overview.starknet_status.configured_networks.join(', ') : 'No configured networks reported.'}</p></div>
    </AdminSectionCard>
  </div>
    ...
    {#if data.overview.recent_activity.length}
      <AdminTable caption="Recent admin activity" columns={['Activity', 'Learner', 'Source', 'Status', 'Timestamp']}>
        {#each data.overview.recent_activity as activity}
          <tr><td><strong>{activity.activity_title}</strong><br /><span class="admin-muted">{activity.activity_type}</span></td><td><a href={'/admin/learners/' + activity.learner_id}>{activity.learner_id}</a></td><td>{activity.source || 'System'}</td><td>{activity.status}</td><td>{activity.timestamp}</td></tr>
        {/each}
      </AdminTable>
    {:else}
