...
<AdminHeader title="Readiness Passport" description="Review protected offchain Passport records without claiming an onchain credential or badge.">{#snippet actions()}<a class="admin-button--secondary" href="/passport" target="_blank" rel="noreferrer">Preview Passport</a>{/snippet}</AdminHeader>
<div class="admin-card-grid"><AdminSectionCard eyebrow="Derivation" title="How Passport status is derived" description="The model combines learning, Lab, and participation evidence."><ol class="admin-derivation"><li>Core lesson evidence establishes proof of learning.</li><li>Lab checkpoints establish proof of practice and safety.</li><li>Workshop signals may add community verification.</li><li>Evidence is summarized into offchain readiness eligibility.</li></ol></AdminSectionCard><AdminSectionCard eyebrow="Chain boundary" title="Credential status" description="The API reports stored anchor status; it does not prove an onchain attestation."><div class="admin-note">No admin action connects a wallet, requests a signature, or performs an onchain write.</div></AdminSectionCard></div>
<AdminSectionCard eyebrow="Protected records" title="Passport records and Evidence Trail" description="Records returned by the protected Admin API.">
  {#if data.apiError}<AdminEmptyState state="error" title="Passport data could not be loaded" detail={data.apiError} />
  {:else if data.passports?.data_source === 'not_available'}<AdminEmptyState title="Passport data is not available" detail="The backend reported data_source: not_available. No fallback records are shown." />
  {:else}
