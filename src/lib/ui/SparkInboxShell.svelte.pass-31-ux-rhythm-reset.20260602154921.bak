<script lang="ts">
  ...
  function saveToggle(message: SparkMessage) {
    ...
    pushToast({ title: wasSaved ? 'Pesan dihapus dari simpanan' : 'Pesan disimpan', copy: message.title, tone: 'info' });
  }

  function readAll() {
    markAllMessagesRead(sparkMessages.map((message) => message.id));
    pushToast({ title: 'Inbox dibaca', copy: 'Semua pesan Spark sudah ditandai dibaca.', tone: 'success' });
  }
</script>

<section class="spark-inbox-hero">
  <div>
    <span class="spark-eyebrow">Pesan</span>
    <h1>Pesan dari Spark.</h1>
    <p>Arahan belajar, keamanan, workshop, Hub, dan Passport dikumpulkan di sini agar kamu tidak kehilangan langkah berikutnya.</p>
    <div class="inbox-hero-actions">
      <SparkButton href="#messages">Lihat pesan</SparkButton>
      <SparkButton href="/dashboard" variant="secondary">Buka Dashboard</SparkButton>
    </div>
  </div>

  <aside class="inbox-summary-card">
    <div><strong>{sparkMessages.length}</strong><span>Total pesan</span></div>
    <div><strong>{unreadCount}</strong><span>Belum dibaca</span></div>
    <div><strong>{savedCount}</strong><span>Tersimpan</span></div>
  </aside>
</section>

<section class="spark-section">
  <SparkCard class="inbox-sync-strip">
    <span><SparkIcon name="shield" size={18} /></span>
    <div>
      <strong>Bahasa pesan dibuat sederhana</strong>
      <p>Pesan Spark harus membantu pengguna memahami langkah berikutnya tanpa istilah teknis yang tidak perlu.</p>
    </div>
    <SparkTrustBadge label="Safety-first" tone="safe" />
  </SparkCard>
</section>

<section id="messages" class="spark-section">
  <SparkCard class="spark-inbox-shell">
    <div class="inbox-tools">
      <label><span>Cari pesan</span><input bind:value={search} type="search" placeholder="Contoh: belajar, wallet, workshop, hub..." /></label>
    ...
    <div class="inbox-summary-line"><span>{sparkMessages.length} pesan</span><span>{unreadCount} belum dibaca</span><span>{savedCount} tersimpan</span></div>
        ...
        {#if filteredMessages.length > 0}
          {#each filteredMessages as message}
            ...
            <button class={`message-row ${message.tone}`} class:active={selectedMessage?.id === message.id} class:unread type="button" onclick={() => openMessage(message)}>
              <span class={`message-icon ${message.tone}`}><SparkIcon name={message.icon} size={17} /></span>
          ...
          {/each}
        {:else}
          <div class="message-empty-state"><SparkIcon name="messages" size={26} /><h3>Pesan tidak ditemukan</h3><p>Coba kata lain atau kembali ke filter Semua.</p></div>
        {/if}
      </div>
      ...
      {#if selectedMessage}
        <article class={`message-detail ${selectedMessage.tone}`}>
          <div class="message-detail-head">
            <span class={`message-detail-icon ${selectedMessage.tone}`}><SparkIcon name={selectedMessage.icon} size={24} /></span>
