...
<AdminHeader title="Safe Practice Lab" description="Observed Lab modules and recent attempt events from the protected Admin API.">
  {#snippet actions()}<a class="admin-button--secondary" href="/lab" target="_blank" rel="noreferrer">Preview Lab</a>{/snippet}
</AdminHeader>

{#if data.apiError}
  <AdminEmptyState state="error" title="Lab data could not be loaded" detail={data.apiError} />
{:else if data.dataSource === 'not_available'}
  <AdminEmptyState title="Lab data is not available" detail="The backend reported data_source: not_available. No fallback activity is shown." />
{:else}
